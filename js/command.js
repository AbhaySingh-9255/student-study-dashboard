/**
 * StudyTrack — Global Command Palette Engine
 */
    ...
    overlay.innerHTML = `
        <div class="modal-card" style="max-width: 600px; margin-top: 10vh; vertical-align: top;">
            <div class="modal-header" style="padding: 12px 16px;">
                <input type="text" id="cmdSearchInput" class="input" placeholder="🔍 Search across subjects, tasks, notes, videos..." style="border: none; font-size: 15px;" oninput="searchCommandPalette()">
            </div>
            <div class="modal-body" id="cmdResultsList" style="max-height: 380px; gap: 6px; padding: 12px;">
                <div style="font-size: 12px; color: var(--text-muted); text-align: center; padding: 20px;">Type to search workspace...</div>
            </div>
        </div>
    `;
...
function searchCommandPalette() {
    ...
    if (!query) {
        resultsContainer.innerHTML = '<div style="font-size: 12px; color: var(--text-muted); text-align: center; padding: 20px;">Type to search workspace...</div>';
        return;
    }
        ...
        try {
            ...
            items.forEach(item => {
                ...
                if (title.toLowerCase().includes(query) || desc.toLowerCase().includes(query)) {
                    matches.push({ title, desc, label: config.label, icon: config.icon, url: config.url });
                }
            });
        } catch (e) {}
    });
        ...
        row.innerHTML = `
            <div class="topic-left">
                ...
                <div style="display: flex; flex-direction: column;">
                    ...
                    <small style="font-size: 11px; color: var(--text-muted);">${escapeCmdHTML(res.desc.substring(0, 50))}</small>
                </div>
            </div>
